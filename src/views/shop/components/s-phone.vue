<template>
  <div class="phone-content">
    <!-- 顶部导航栏 -->
    <div class="phone-top optional"
      :class="{ selected: 'page' === props.selectedIndex, [props.data.page.style.titleTextColor]: true }"
      :style="{ backgroundColor: props.data.page.style.titleBackgroundColor }" @click="handelClickItem('page')">
      <p class="title" :style="{ color: props.data.page.style.titleTextColor }">{{ props.data.page.params.title }}</p>
    </div>
    <!-- 内容区域 -->
    <div class="phone-main">
      <draggable :list="props.data.items" class="content" @update="handelDragItem"
        v-bind="{ animation: 120, filter: '.undrag' }">
        <!-- 内容元素 -->
        <div class="devise-item optional" v-for="(item, index) in props.data.items" :key="index" @click="handelClickItem(index)"
          :class="{ selected: index === props.selectedIndex, undrag: inArray(item.type, undragList) }"
          :style="renderItemStyle(item)">
          <!-- 图片轮播 -->
          <div v-if="item.type == 'banner'" class="diy-banner">
            <img v-for="(dataItem, dataIdx) in item.data" :key="`${index}_${dataIdx}_img`" v-show="dataIdx <= 1"
              :src="dataItem.imgUrl" />
            <div class="dots" :class="item.style.btnShape">
              <div v-for="(dataItem, dataIdx) in item.data" :key="`${index}_${dataIdx}_dots`"
                :style="{ background: item.style.btnColor }" class="dots-item"></div>
            </div>
          </div>

          <!-- 图片组 -->
          <div v-else-if="item.type == 'image'" class="diy-image"
            :style="{ paddingBottom: item.style.paddingTop + 'px', background: item.style.background }">
            <div class="item-image" v-for="(dataItm, dataIdx) in item.data" :key="`${index}_${dataIdx}`"
          ...
          <!-- 图片橱窗 -->
          <div v-else-if="item.type == 'window'" class="diy-window"
            ...
            <ul v-if="item.style.layout > -1" class="data-list clearfix" :class="`avg-sm-${item.style.layout}`">
              <li v-for="(window, dataIdx) in item.data" :key="`${index}_${dataIdx}`" class="data-item"
          ...
          <div v-else-if="item.type == 'article'" class="diy-article">
            <div class="diy-article">
              <div class="article-item"
                ...
                :key="`${index}_${dataIdx}`" :class="`show-type__${dataItm.show_type}`">
                <!-- 小图模式 -->
          ...
          <div v-else-if="item.type == 'search'" class="diy-search">
            <div class="inner" :class="item.style.searchStyle">
              <div class="search-input" :style="{ textAlign: item.style.textAlign }">
                <component class="search-icon" :is="'Search'"></component>
          ...
          <div v-else-if="item.type == 'notice'" class="diy-notice" :style="{ padding: `${item.style.paddingTop}px 0` }">
            <div class="notice-body" :style="{ background: item.style.background, color: item.style.textColor }">
              <div class="notice__icon">
                <component class="notice-icon" :is="'Search'"></component>
              </div>
          ...
          <!-- 导航组 -->
          <div v-else-if="item.type == 'navBar'" class="diy-navBar"
            ...
            <ul class="data-list clearfix" :class="`avg-sm-${item.style.rowsNum}`">
              <li class="item-nav" v-for="(dataItm, dataIdx) in item.data" :key="`${index}_${dataIdx}`">
                <div class="item-image">
                  <img :src="dataItm.imgUrl" />
                </div>
            ...
            <ul class="goods-list clearfix" :class="['display__' + item.style.display, 'column__' + item.style.column]">
              <li class="goods-item"
                  ...
                  <div class="flex">
                    ...
                    <div class="goods-item_right">
                      ...
                      <div class="goods-item_desc">
                        <!-- 商品卖点 -->
                        <div v-if="item.style.show.includes('sellingPoint')" class="desc-selling_point oneline-hide">
                          <span>{{ dataItm.selling_point }}</span>
                        </div>
                        <!-- 商品销量 -->
                        <div v-if="item.style.show.includes('goodsSales')" class="desc-goods_sales oneline-hide">
                ...
                <template v-else>
                  ...
                  <div class="detail">
                    <p v-if="item.style.show.includes('goodsName')" class="goods-name twolist-hidden">{{
                    ...
                    <p class="detail-price">
                      ...
                      <span v-if="item.style.show.includes('linePrice') && dataItm.line_price_min > 0" class="line-price">
                        <span class="small-unit">¥</span>
          ...
          <!-- 辅助空白 -->
          <div v-else-if="item.type == 'blank'" class="diy-blank"
            :style="{ height: `${item.style.height}px`, background: item.style.background }"></div>

          <!-- 辅助线 -->
          <div v-else-if="item.type == 'guide'" class="diy-guide"
          ...
          <div v-else-if="item.type == 'service'" class="diy-service" :style="{ opacity: item.style.opacity / 100 }">
            <div class="service-icon">
              <img class="image" :src="item.params.image" />
            </div>
          </div>

          <!-- 富文本 -->
          <div v-else-if="item.type == 'richText'" class="diy-richText"
            :style="{ background: item.style.background, padding: `${item.style.paddingTop}px ${item.style.paddingLeft}px` }"
            v-html="item.params.content"></div>
...
// 禁止拖拽的组件
const emits = defineEmits(["onEditer", "onDeleleItem"]);
const undragList = ref(['service'])
...
const inArray = (search: string, array: any[]) => {
...
/**
       ...
       * @param e
       */
const handelDragItem = (e: any) => {
  emits('onEditer', e.newIndex)
}
 ...
 * @param index
 */
const handleDeleleItem = (index: number) => {
  emits('onDeleleItem', index)
}

// 渲染组件外层容器的样式
const renderItemStyle = (item: any) => {
